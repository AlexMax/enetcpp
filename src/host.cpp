                              ...
                              uint32_t incomingBandwidth, uint32_t outgoingBandwidth)
{
    ...
    if (!channelLimit || channelLimit > ENet::PROTOCOL_MAXIMUM_CHANNEL_COUNT)
    {
        channelLimit = ENet::PROTOCOL_MAXIMUM_CHANNEL_COUNT;
    }
    else if (channelLimit < ENet::PROTOCOL_MINIMUM_CHANNEL_COUNT)
    {
        channelLimit = ENet::PROTOCOL_MINIMUM_CHANNEL_COUNT;
    }
    ...
    host->mtu = ENet::HOST_DEFAULT_MTU;
    ...
    host->duplicatePeers = ENet::PROTOCOL_MAXIMUM_PEER_ID;
...
ENet::Peer *ENet::host_connect(ENet::Host *host, const ENet::Address *address, size_t channelCount, uint32_t data)
{
    ...
    if (channelCount < ENet::PROTOCOL_MINIMUM_CHANNEL_COUNT)
    {
        channelCount = ENet::PROTOCOL_MINIMUM_CHANNEL_COUNT;
    }
    else if (channelCount > ENet::PROTOCOL_MAXIMUM_CHANNEL_COUNT)
    {
        channelCount = ENet::PROTOCOL_MAXIMUM_CHANNEL_COUNT;
    }
    ...
    {
        currentPeer->windowSize = ENet::PROTOCOL_MAXIMUM_WINDOW_SIZE;
    }
    else
    {
        currentPeer->windowSize =
            (host->outgoingBandwidth / ENet::PEER_WINDOW_SIZE_SCALE) * ENet::PROTOCOL_MINIMUM_WINDOW_SIZE;
    }

    if (currentPeer->windowSize < ENet::PROTOCOL_MINIMUM_WINDOW_SIZE)
    {
        currentPeer->windowSize = ENet::PROTOCOL_MINIMUM_WINDOW_SIZE;
    }
    else if (currentPeer->windowSize > ENet::PROTOCOL_MAXIMUM_WINDOW_SIZE)
    {
        currentPeer->windowSize = ENet::PROTOCOL_MAXIMUM_WINDOW_SIZE;
    }
...
void ENet::host_broadcast(ENet::Host *host, uint8_t channelID, ENet::Packet *packet)
{
    ...
    for (currentPeer = host->peers; currentPeer < &host->peers[host->peerCount]; ++currentPeer)
    {
        if (currentPeer->state != ENet::PEER_STATE_CONNECTED)
        {
            continue;
        }
...
void ENet::host_bandwidth_throttle(ENet::Host *host)
{
    ...
    if (elapsedTime < ENet::HOST_BANDWIDTH_THROTTLE_INTERVAL)
    {
        return;
    }
    ...
    while (peersRemaining > 0 && needsAdjustment != 0)
    {
        ...
        {
            throttle = ENet::PEER_PACKET_THROTTLE_SCALE;
        }
        else
        {
            throttle = (bandwidth * ENet::PEER_PACKET_THROTTLE_SCALE) / dataTotal;
        }
        ...
        for (peer = host->peers; peer < &host->peers[host->peerCount]; ++peer)
        {
            ...
            if ((throttle * peer->outgoingDataTotal) / ENet::PEER_PACKET_THROTTLE_SCALE <= peerBandwidth)
            {
                continue;
            }

            peer->packetThrottleLimit = (peerBandwidth * ENet::PEER_PACKET_THROTTLE_SCALE) / peer->outgoingDataTotal;
